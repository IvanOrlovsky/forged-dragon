...
export default function Gallery({ tabs, imagesByTab }: ContentType) {
	...
	return (
		<div className="flex flex-col">
			<ul className="bg-light-primary dark:bg-dark-primary p-1 sm:p-3 rounded-lg w-fit self-center flex flex-wrap justify-center sm:gap-3 text-sm sm:text-xl font-medium text-center dark:text-light-text text-dark-text">
				{tabs.map((tab) => (
					<li key={tab}>
						<button
							onClick={() => setActiveTab(tab)}
							className={`inline-block px-4 py-3 rounded-lg transition-colors duration-300 ${
								activeTab === tab
